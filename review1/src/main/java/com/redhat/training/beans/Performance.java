...
public class Performance {
	...
	private ConcurrentMap<Integer, Map<String, Long>> stats = new ConcurrentHashMap<Integer, Map<String, Long>>(
			20);
	...
	public void dumpStats() {
		for (Entry<Integer, Map<String, Long>> batch : stats.entrySet()) {
			System.out.println(String.format("Batch %d processed in %d ms.", batch.getKey(),
					batch.getValue().get("stop") - batch.getValue().get("start")));
		}
	}
